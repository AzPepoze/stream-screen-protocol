...
func NewEncoder(cfg map[string]interface{}) (*Encoder, error) {
	...
	return &Encoder{fps: fps, bitrate: bitrate}, nil
}
...
func (e *Encoder) Encode(rgbaData []byte, width, height int) ([]byte, error) {
	...
	if len(rgbaData) != width*height*4 {
		return nil, fmt.Errorf("amf encoder: invalid RGBA frame size got=%d expected=%d", len(rgbaData), width*height*4)
	}
	if err := e.ensureSession(width, height); err != nil {
		return nil, fmt.Errorf("amf encoder: persistent session: %w", err)
	}
