...
func (r *ClientReceiver) appsrcLoop() {
	...
	for {
		...
		case f, ok := <-r.frameChan:
			...
			// log.Printf("Client: appsrc pushing frame=%d size=%d", f.Seq, len(f.Data))
			if r.appsrc == nil {
				continue
			}

			buffer := gst.NewBufferFromBytes(f.Data)
			buffer.SetPresentationTimestamp(timestamp)
			buffer.SetDuration(r.frameInterval)

			ret := r.appsrc.PushBuffer(buffer)
			if ret != gst.FlowOK {
				log.Printf("appsrc push returned %v", ret)
			}

			timestamp += r.frameInterval
		}
	}
}
...
func (r *ClientReceiver) busLoop() {
	bus := r.pipeline.GetBus()
	if bus == nil {
		log.Printf("Client: failed to get pipeline bus")
		return
	}
	...
	for {
		...
		default:
			...
			case gst.MessageError:
				err := msg.ParseError()
				log.Printf("Client: GStreamer error: %v (debug: %s)", err.Error(), err.DebugString())
			case gst.MessageStateChanged:
				oldState, newState := msg.ParseStateChanged()
				log.Printf("Client: pipeline state changed: %v -> %v", oldState, newState)
			case gst.MessageEOS:
				log.Printf("Client: GStreamer EOS")
			default:
